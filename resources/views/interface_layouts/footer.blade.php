<footer id="footer" class="region region-footer">
    <div id="block-itg-ads-ads-overlay-1x1" class="block block-itg-ads first odd">
        <div id="zt_767460_1">
        </div>
    </div>
    <div id="block-block-48" class="block block-block even">
        <div class="close-foru"><i class="arrow up"></i></div>
        <div class="fixed__foru__nav hide">
            <div class="foryou__nav">
                <a href="#">
                    <span class="icon">
                        <img src="data:image/png;base64," alt="">
                    </span>
                    <span class="nav__text">MY NEWS FEED</span>
                </a>
            </div>
        </div>
        <script>
            document.addEventListener("DOMContentLoaded", function () {
                jQuery(".close-foru").click(function () {
                    jQuery("i.arrow").toggleClass("down");
                    jQuery(".fixed__foru__nav").toggleClass("downfeed");
                });
            });
        </script>
    </div>
    <div id="block-itg-layout-manager-footer-block" class="block block-itg-layout-manager odd">
        <section class="footer-top">
            <div class="container">
                <div class="footer-top-link"> INDIATODAY.IN</div>
                <div class="footer-social-link">
                    <ul>
                        <li>
                            <a rel="noopener noreferrer" target="_blank" href="#" title="">
                                <i class="fa fa-facebook"></i>
                            </a>
                        </li>
                        <li>
                            <a rel="noopener noreferrer" target="_blank" href="#" title="">
                                <i class="fa fa-twitter"></i>
                            </a>
                        </li>
                        <li>
                            <a href="#" title=""><i class="fa fa-rss"></i></a>
                        </li>
                    </ul>
                </div>
                <div class="footer-expand-icon"></div>
            </div>
        </section>
        <section class="footer-toggle">
            <section class="footer-mid">
                <div class="container">
                    <div id="block-itg-ads-ads-super-banner-bottomnav728x90" class="block block-itg-ads first odd">
                        <div class="adtext">advertisement</div>
                        <div id="div-gpt-ad-1514896195242-2" data-google-query-id="CPKLhcat0_ACFTmC6QUdT0kNQQ">
                            <div id="google_ads_iframe_/1007232/Indiatoday_HP_BTF_728x90-300x250_0__container__" style="border: 0 none;"></div>
                        </div>
                    </div>
                </div>
            </section>
            <section class="footer-bottom">
                <div class="container">
                    <div class="row multiple-items-footer slick-initialized slick-slider">
                        <div aria-live="polite" class="slick-list draggable">
                            <div class="slick-track" role="listbox" style="opacity: 1; width: 30000px; transform: translate3d(0px, 0px, 0px);">
                                <div class="cell slick-slide slick-current slick-active" tabindex="-1" style="" data-slick-index="0" aria-hidden="false">
                                    <h4>Publications:</h4>
                                    <ul class="menu">
                                        <li class="menu__item is-leaf first leaf">
                                            <a href="#" class="menu__link" tabindex="0">Business Today</a>
                                        </li>
                                        <li class="menu__item is-leaf leaf">
                                            <a href="#" class="menu__link" tabindex="0">Cosmopolitan</a>
                                        </li>
                                        <li class="menu__item is-leaf leaf">
                                            <a href="#" class="menu__link" tabindex="0">India Today - Hindi</a>
                                        </li>
                                        <li class="menu__item is-leaf leaf">
                                            <a href="#">India Today</a>
                                        </li>
                                        <li class="menu__item is-leaf leaf">
                                            <a href="#" class="menu__link" tabindex="0">Mail Today</a>
                                        </li>
                                        <li class="menu__item is-leaf leaf">
                                            <a href="#" class="menu__link" tabindex="0">Money Today</a>
                                        </li>
                                        <li class="menu__item is-leaf leaf">
                                            <a href="#" class="menu__link" tabindex="0">Reader's Digest</a>
                                        </li>
                                        <li class="menu__item is-leaf last leaf">
                                            <a href="#" class="menu__link" tabindex="0">Time</a>
                                        </li>
                                    </ul>
                                </div>
                                <div class="cell slick-slide slick-active" tabindex="-1" style="" data-slick-index="1" aria-hidden="false">
                                    <h4>Television:</h4>
                                    <ul class="menu">
                                        <li class="menu__item is-leaf first leaf">
                                            <a href="#" class="menu__link" tabindex="0">Aaj Tak</a>
                                        </li>
                                        <li class="menu__item is-leaf leaf">
                                            <a href="#" class="menu__link" tabindex="0">Delhi Aaj Tak</a>
                                        </li>
                                        <li class="menu__item is-leaf leaf">
                                            <a href="#" class="menu__link" tabindex="0">India Today TV</a>
                                        </li>
                                        <li class="menu__item is-leaf last leaf">
                                            <a href="#" class="menu__link" tabindex="0">Tez</a>
                                        </li>
                                    </ul>
                                    <h4>Radio:</h4>
                                    <ul class="menu">
                                        <li class="menu__item is-leaf first last leaf">
                                            <a href="#" class="menu__link" tabindex="0">Ishq FM</a>
                                        </li>
                                    </ul>
                                </div>
                                <div class="cell slick-slide slick-active" tabindex="-1" style="" data-slick-index="2" aria-hidden="false">
                                    <h4>Education:</h4>
                                    <ul class="menu">
                                        <li class="menu__item is-leaf first leaf">
                                            <a href="#" class="menu__link" tabindex="0">India Today Education</a>
                                        </li>
                                        <li class="menu__item is-leaf leaf">
                                            <a href="#" class="menu__link" tabindex="0">Vasant Valley</a>
                                        </li>
                                        <li class="menu__item is-leaf leaf">
                                            <a href="#" target="_blank" class="menu__link" tabindex="0">Best Colleges India 2018</a>
                                        </li>
                                        <li class="menu__item is-leaf last leaf">
                                            <a href="#" target="_blank" class="menu__link" tabindex="0">Best Universities India 2018</a>
                                        </li>
                                    </ul>
                                    <h4>Syndications:</h4>
                                    <ul class="menu">
                                        <li class="menu__item is-leaf first leaf">
                                            <a href="#" rel="noopener noreferrer" class="menu__link" tabindex="0">India content</a>
                                        </li>
                                        <li class="menu__item is-leaf last leaf">
                                            <a href="#" rel="noopener noreferrer" class="menu__link" tabindex="0">Headlines Today</a>
                                        </li>
                                    </ul>
                                </div>
                                <div class="cell slick-slide slick-active" tabindex="-1" style="" data-slick-index="3" aria-hidden="false">
                                    <h4>Events:</h4>
                                    <ul class="menu">
                                        <li class="menu__item is-leaf first leaf">
                                            <a href="#" class="menu__link" tabindex="0">Agenda Aajtak</a>
                                        </li>
                                        <li class="menu__item is-leaf leaf">
                                            <a href="#" class="menu__link" tabindex="0">India Today Conclave</a>
                                        </li>
                                        <li class="menu__item is-leaf leaf">
                                            <a href="#" class="menu__link" tabindex="0">Robb Report India 2018</a>
                                        </li>
                                        <li class="menu__item is-leaf leaf">
                                            <a href="#" rel="noopener noreferrer" target="_blank" class="menu__link" tabindex="0">Sahitya Aaj Tak</a>
                                        </li>
                                        <li class="menu__item is-leaf last leaf">
                                            <a href="#" rel="noopener noreferrer" target="_blank" class="menu__link" tabindex="0">The Red Lab</a>
                                        </li>
                                    </ul>
                                    <h4>Distribution:</h4>
                                    <ul class="menu">
                                        <li class="menu__item is-leaf first last leaf">
                                            <a href="#" rel="noopener noreferrer" target="_blank" class="menu__link" tabindex="0">Rate Card</a>
                                        </li>
                                    </ul>
                                </div>
                                <div class="cell slick-slide slick-active" tabindex="-1" style="" data-slick-index="4" aria-hidden="false">
                                    <h4>Printing:</h4>
                                    <ul class="menu">
                                        <li class="menu__item is-leaf first last leaf">
                                            <a href="#" class="menu__link" tabindex="0">Thomson Press</a>
                                        </li>
                                    </ul>
                                    <h4>Welfare:</h4>
                                    <ul class="menu">
                                        <li class="menu__item is-leaf first last leaf">
                                            <a href="#" rel="noopener noreferrer" class="menu__link" tabindex="0">Care Today</a>
                                        </li>
                                    </ul>
                                    <h4>Music:</h4>
                                    <ul class="menu">
                                        <li class="menu__item is-leaf first last leaf">
                                            <a href="#" class="menu__link" tabindex="0">Music Today</a>
                                        </li>
                                    </ul>
                                </div>
                                <div class="cell slick-slide slick-active" tabindex="-1" style="" data-slick-index="5" aria-hidden="false">
                                    <h4>Useful Links :</h4>
                                    <ul class="menu">
                                        <li class="menu__item is-leaf first leaf">
                                            <a href="#" class="menu__link" tabindex="0">Partners</a>
                                        </li>
                                        <li class="menu__item is-leaf leaf">
                                            <a href="#" class="menu__link" tabindex="0">Press Release</a>
                                        </li>
                                        <li class="menu__item is-leaf leaf">
                                            <a href="#" title="Sitemap" class="menu__link" tabindex="0">Sitemap</a>
                                        </li>
                                        <li class="menu__item is-leaf leaf">
                                            <a href="#" class="menu__link" tabindex="0">News</a>
                                        </li>
                                        <li class="menu__item is-leaf leaf">
                                            <a href="#" class="menu__link" tabindex="0">Newsletter</a>
                                        </li>
                                        <li class="menu__item is-leaf leaf">
                                            <a href="#" rel="noopener noreferrer" target="_blank" class="menu__link" tabindex="0">Privacy Policy</a>
                                        </li>
                                        <li class="menu__item is-leaf last leaf">
                                            <a href="#" target="_blank" class="menu__link" tabindex="0">Correction Policy</a>
                                        </li>
                                    </ul>
                                </div>
                            </div>
                        </div>
                    </div>
                </div>
            </section>
        </section>
        <div id="widget-ajex-loader" style="display: none">
            <img class="lazyload widget-loader" data-src="https://akm-img-a-in.tosshub.com/indiatoday/../sites/all/themes/itg/images/loading.gif" alt="Loading..." src="https://akm-img-a-in.tosshub.com/indiatoday/../sites/all/themes/itg/images/loading.gif">
        </div>
        <div id="iframe-display" style="display: none"></div>
    </div>
    <div id="block-block-13" class="block block-block even">
        <div class="container">
            <div class="app_menu">
                <h6>Download App</h6>
                <ul>
                    <li>
                        <a rel="noopener noreferrer" href="#" target="_blank" title="Andriod App">
                            <img alt="Andriod App" class="lazyloaded" data-src="data:image/png;base64," src="data:image/png;base64,">
                        </a>
                    </li>
                    <li>
                        <a rel="noopener noreferrer" href="#" target="_blank" title="IOS App">
                            <img alt="IOS App" class="lazyloaded" data-src="data:image/png;base64," src="data:image/png;base64,">
                        </a>
                    </li>
                    <li>
                        <a rel="noopener noreferrer" href="#" target="_blank" title="SmartTv App">
                            <img alt="SmartTv App" class="lazyloaded" data-src="data:image/png;base64," src="data:image/png;base64,">
                        </a>
                    </li>
                </ul>
            </div>
        </div>
        <section class="footer-copyright-new">
            <div class="container">
                <p>Copyright © 2021 Living Media India Limited. For reprint rights:
                    <a rel="noopener noreferrer" href="#" target="_blank">Syndications Today</a>
                </p>
            </div>
        </section>
    </div>
    <div id="block-block-55" class="block block-block last odd">
    </div>
</footer>
